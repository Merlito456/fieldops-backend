...
app.post('/api/auth/vendor/register', async (req, res) => {
  ...
  try {
    const v = req.body;
    await pool.query(
      `INSERT INTO vendors (id, username, password, full_name, company, contact_number, photo_url, id_number, specialization) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
...
app.post('/api/auth/vendor/login', async (req, res) => {
  ...
  try {
    ...
    if (rows.length > 0) {
      const v = rows[0];
      res.json({ id: v.id, username: v.username, fullName: v.full_name, company: v.company, contactNumber: v.contact_number, photo: v.photo_url, idNumber: v.id_number, specialization: v.specialization });
...
app.get('/api/sites', async (req, res) => {
  ...
  try {
    ...
    const mapped = rows.map(s => ({
      id: s.id, name: s.name, type: s.type, address: s.address, gpsCoordinates: s.gps_coordinates, priority: s.priority, lastMaintenanceDate: s.last_maintenance_date, nextMaintenanceDate: s.next_maintenance_date, assetPhoto: s.asset_photo_url, towerHeight: s.tower_height, equipmentBrand: s.equipment_brand, keyStatus: s.key_status, caretaker: s.caretaker, caretakerContact: s.caretaker_contact, pendingVisitor: safeParse(s.pending_visitor), currentVisitor: safeParse(s.current_visitor), visitorHistory: safeParse(s.visitor_history) || [], pendingKeyLog: safeParse(s.pending_key_log), currentKeyLog: safeParse(s.current_key_log), keyHistory: safeParse(s.key_history) || [], accessAuthorized: !!s.access_authorized, keyAccessAuthorized: !!s.key_access_authorized
    }));
    res.json(mapped);
  } catch (err) { res.status(500).json({ error: err.message }); }
});
...
app.post('/api/sites', async (req, res) => {
  ...
  const s = req.body;
  try {
    await pool.query(`INSERT INTO sites (id, name, type, address, gps_coordinates, priority, last_maintenance_date, next_maintenance_date, asset_photo_url, caretaker, caretaker_contact, tower_height, tower_type, equipment_brand, sectors) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [s.id, s.name, s.type, s.address, s.gpsCoordinates, s.priority, s.lastMaintenanceDate, s.nextMaintenanceDate, s.assetPhoto, s.caretaker, s.caretakerContact, s.towerHeight, s.towerType, s.equipmentBrand, s.sectors]);
    res.json(s);
  } catch (err) { res.status(500).json({ error: 'Database save failed: ' + err.message }); }
});
...
app.put('/api/sites/:id', async (req, res) => {
  ...
  const s = req.body;
  try {
    await pool.query(`UPDATE sites SET name=?, type=?, address=?, gps_coordinates=?, priority=?, asset_photo_url=?, caretaker=?, caretaker_contact=?, tower_height=?, tower_type=?, equipment_brand=?, sectors=? WHERE id=?`, [s.name, s.type, s.address, s.gpsCoordinates, s.priority, s.assetPhoto, s.caretaker, s.caretakerContact, s.towerHeight, s.towerType, s.equipmentBrand, s.sectors, req.params.id]);
    res.json(s);
  } catch (err) { res.status(500).json({ error: err.message }); }
});
...
app.post('/api/access/checkin/:siteId', async (req, res) => {
  ...
  try {
    ...
    if (rows.length > 0 && rows[0].pending_visitor) {
      ...
      await pool.query('UPDATE sites SET current_visitor = ?, pending_visitor = NULL, access_authorized = 0 WHERE id = ?', [JSON.stringify(visitor), req.params.siteId]);
      res.json(visitor);
    } else res.status(404).json({ error: 'No pending access request found.' });
  } catch (err) { res.status(500).json({ error: err.message }); }
});
...
app.post('/api/access/checkout/:siteId', async (req, res) => {
  ...
  try {
    ...
    if (rows.length > 0 && rows[0].current_visitor) {
      ...
      await pool.query('UPDATE sites SET current_visitor = NULL, visitor_history = ? WHERE id = ?', [JSON.stringify(updatedHistory), req.params.siteId]);
      res.json({ success: true });
    } else res.status(404).json({ error: 'No active session found.' });
  } catch (err) { res.status(500).json({ error: err.message }); }
});
...
app.post('/api/keys/confirm/:siteId', async (req, res) => {
  ...
  try {
    ...
    if (rows.length > 0 && rows[0].pending_key_log) {
      ...
      await pool.query('UPDATE sites SET current_key_log = ?, pending_key_log = NULL, key_access_authorized = 0, key_status = "Borrowed" WHERE id = ?', [JSON.stringify(keyLog), req.params.siteId]);
      res.json(keyLog);
    } else res.status(404).json({ error: 'No pending key request found.' });
  } catch (err) { res.status(500).json({ error: err.message }); }
});
...
app.post('/api/keys/return/:siteId', async (req, res) => {
  ...
  try {
    ...
    if (rows.length > 0 && rows[0].current_key_log) {
      ...
      const updatedHistory = [finished, ...history].slice(0, 20);
      await pool.query('UPDATE sites SET current_key_log = NULL, key_history = ?, key_status = "Available" WHERE id = ?', [JSON.stringify(updatedHistory), req.params.siteId]);
      res.json({ success: true });
    } else res.status(404).json({ error: 'No active key record found.' });
  } catch (err) { res.status(500).json({ error: err.message }); }
});
...
app.get('/api/tasks', async (req, res) => {
  ...
  try {
    ...
    res.json(rows.map(t => ({ id: t.id, title: t.title, description: t.description, siteId: t.site_id, assignedTo: t.assigned_to, status: t.status, priority: t.priority, type: t.type, scheduledDate: t.scheduled_date })));
  } catch (err) { res.json([]); }
});
...
app.post('/api/tasks', async (req, res) => {
  ...
  try {
    const t = req.body;
    await pool.query(`INSERT INTO tasks (id, title, description, site_id, assigned_to, status, priority, type, scheduled_date) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`, [t.id, t.title, t.description, t.siteId, t.assignedTo, t.status, t.priority, t.type, t.scheduledDate]);
    res.json(t);
  } catch (err) { res.status(500).json({ error: err.message }); }
});
...
app.listen(PORT, () => {
  console.log(`FIELDOPS PRO API: ACTIVE ON PORT ${PORT}`);
});
